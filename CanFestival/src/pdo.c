      ...
      case 5:                  /* Changed event time */
        {
          ...
          /* Call  PDOEventTimerAlarm for this TPDO, this will trigger emission et reset timers */          //事件定时器作用
          PDOEventTimerAlarm (d, numPdo);
          return 0;
        }
...
void
PDOInit (CO_Data * d)               //协议栈逻辑会在PDO初始化时将参数更改回调函数赋值（主要TPDO？？？），与设计逻辑有冲突,所以在此函数调用之后再将存储值幅值，
{
